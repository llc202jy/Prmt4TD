...
class BertTrainer:
    def __init__(self, train_df, test_df, data_type):
        ...
        self.result_file = f'{folder}/results/bert/{data_type}/results.txt'
        self.p_value_file = f'{folder}/results/bert/{data_type}/p_value.json'
    ...
    def get_training_arguments(self):
        return TrainingArguments(
            output_dir=f"{folder}/results/bert/{self.data_type}",  # 每折的输出目录
            ...
            weight_decay=0.01,
            logging_dir=f"{folder}/logs/bert/{self.data_type}",  # 日志目录
            logging_steps=10,
    ...
    def test_model(self, test_df):
        # 加载训练好的模型
        # 替换为实际折数路径
        model_path = f"{folder}/results/bert/{self.data_type}/checkpoint-{self.get_best_checkpoint()}"
        model = BertForSequenceClassification.from_pretrained(model_path)

        # 加载新数据
        # new_data = pd.read_csv(test_file_path, sep='\t')  # 替换为实际路径
        new_data = test_df  # 替换为实际路径
    ...
    def get_best_checkpoint(self):
        ...
        model_dir = f"{folder}/results/bert/{self.data_type}"
...
if __name__ == '__main__':
    ...
    trainer = BertTrainer(train_df, test_df, 'TacticData')
    # 验证平衡数据集的20%
    test_df = pd.read_csv('../dataset/TacticData_拆分/TacticData_val.tsv', sep='\t')
