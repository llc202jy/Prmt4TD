...
def main_LLM(train_df, data_type, gpt_type):
    # 准备输出文件
    output_file1 = f'train_result/llm_result/onlyLLM/{data_type}/{gpt_type}/results_PBas.csv'
    os.makedirs(os.path.dirname(output_file1), exist_ok=True)
    output_file2 = f'train_result/llm_result/onlyLLM/{data_type}/{gpt_type}/results_PABas.csv'
    os.makedirs(os.path.dirname(output_file2), exist_ok=True)
    with open(output_file1, mode='w', newline='', encoding='utf-8') as file1, open(output_file2, mode='w', newline='', encoding='utf-8') as file2:
        writer1 = csv.DictWriter(file1, fieldnames=["Input Code", "True Label", "Generated Prompt", "LLM Prediction"])
        writer2 = csv.DictWriter(file2, fieldnames=["Input Code", "True Label", "Generated Prompt", "LLM Prediction"])
        writer1.writeheader()
        writer2.writeheader()
        start_time = time.time()
        print(f"开始处理测试集，开始时间：{datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}")
        for index, row in train_df.iterrows():
            prompt = "Please analyze the input code snippet, and determine the most appropriate architectural tactic label from {'audit', 'authenticate', 'heartbeat', 'pooling', 'scheduler', 'unrelated'}, providing your reasons."
            ...
            if gpt_type == "qwen":
                ...
                from LLM.call_deepseek import call_deepseekmodel
                gpt_response = call_deepseekmodel(prompt)
            writer1.writerow({
                "Input Code": row['text'],
            ...
            prompt = (
                "Step1: Please understand the behavior and purpose of the input code snippet.\n"
                "Step2: Determine the unique true label of this code from {'audit', 'authenticate', 'heartbeat','pooling', 'scheduler', 'unrelated'}.\n"
                "Step3: Please give your reasoning.")
            prompt = prompt + row['text']
            gpt_response = None
            print("开始调用模型")
            if gpt_type == "qwen":
...
def main_do(my_model, train_df, test_df, data_type, gpt_type, model_type):
    ...
    test_df_c = test_df
    embedding_test = get_embedding_data(test_df_c, 'test_data')

    # 准备输出文件
    output_file = f'llm_result/{model_type}_{gpt_type}/{data_type}/results.csv'
    ...
    with open(output_file, mode='a', newline='', encoding='utf-8') as file:
        ...
        for index, row in test_df_c.iterrows():
            ...
            # 筛选出符合条件的标签
            # ['authenticate']
            # [{'label': 'pooling', 'most_similar_code': 'xx','similarity': np.float64(0.4330363503757007)}]
            results = find_similar_code_in_dataset(train_df, input_code, filtered_labels,
                                                   embedding_train,
                                                   embedding_test[index])
            # 转换成 code_text：相似代码块，true_label：相似代码块的真实标签
            # predicted_label：预测标签，predicted_probability：预测概率
...
def main(data_type, model_type, gpt_type, only_train):
    train_data_file = ''
    test_data_file = ''
    ...
    if gpt_type == "":
        if not only_train:
            model.test_model(test_df)
    else:
        if model_type == "":
            train_df = train_df[train_df['label'] != 'unrelated']
            train_df = train_df.reset_index(drop=True)
            main_LLM(train_df, data_type, gpt_type)
        else:
            main_do(model, train_df, test_df, data_type, gpt_type, model_type)


if __name__ == '__main__':
    data_type = "TacticData"
    only_train = False
    model_type = "SVM"
    gpt_type = "deepseek"
    prompt_type = "step"
    main(data_type, model_type, gpt_type, only_train)
