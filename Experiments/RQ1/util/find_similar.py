...
def find_most_similar_code(input_code, filtered_df, input_code_embedding, train_embedding):
    ...
    if not filtered_df.empty:
        ...
        return most_similar_row['text'], most_similar_row['similarity']
    else:
        print("未找到有效的相似代码片段")
        return None, None
...
def find_similar_code_in_dataset(train_df, input_code, labels, train_embedding, input_code_embedding):
    ...
    for label in labels:
        ...
        if not filtered_train_df.empty:
            most_similar_code, similarity = find_most_similar_code(input_code, filtered_train_df, input_code_embedding,
                                                                   train_embedding)
            results.append({
                'label': label,
